...
- `src/app/staff/` - The real, database-backed staff area (Phase 1.2, see `CLAUDE.md`): `login/` (email + password, then mandatory TOTP — `StaffLoginForm.tsx` is the one client component, using `useActionState` against the Server Actions in `src/server/auth/actions.ts`), `page.tsx` (the protected `/staff` shell — deliberately plain; `src/components/admin/` is the polished design preview P1.3 wires up to this real data next). Nav and Footer hide themselves here the same way they do on `/admin`
...
- `src/data/*.ts` - What's left after Phase 1.3 moved content and prices to the database (see `content.ts` below): `sessions.ts`/`packs.ts` keep only presentation constants with no row to live in (`INCLUDED`, `OFF_PEAK`, `PARTNER_SESSIONS`, `GIFT_VOUCHER`, `CORPORATE`, `formatRWF`); `journal.ts` (blog articles — no schema table, not in the owner's brief) and `images.ts` (Pinterest/Unsplash placeholders — real photography is a Phase 0 decision) stay static for now. `site.ts` is gone; everything in it moved to the database
- `src/server/db/` - The backend (Phase 1, see `CLAUDE.md`). `schema/*.ts` is the data model — read `docs/database.md` before touching it, especially before changing `bookings` or `ledger_entries`. `migrations/` includes hand-written SQL (`0001_constraints_and_guards.sql`) alongside `drizzle-kit generate` output; both apply through `pnpm db:migrate`. `client.ts` is the app's DB connection — a lazily-opened `Proxy` (`server-only`-guarded; never imported by a script run outside Next, which needs its own connection; see the comment in `seed.ts`) so a missing `DATABASE_URL` fails the first real query, not `next build`'s routine walk of every route's module graph. `content.ts` is the read side of Phase 1.3: cached functions (`getSiteConfig`, `getSessionTypes`, `getPackProducts`, `getFaqs`, `getContentBlocks`, ...) every public page calls instead of importing `src/data/*.ts`

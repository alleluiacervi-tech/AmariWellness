...
- `src/app/account/` - `page.tsx` (real upcoming/past bookings for the signed-in client — `requireClientPage()`), `login/` (`AccountLoginForm.tsx`, phone + one-time code via `useActionState`, the same pattern as `StaffLoginForm.tsx` but without a password or MFA-enrollment step)
...
- `src/server/notify/sms.ts` - The one-time-code delivery adapter (Phase 1.4b), selected by `SMS_PROVIDER` (`.env.example`) — same pattern as `payments/`. `consoleSmsAdapter` logs instead of sending; `src/server/client-auth/actions.ts` echoes the code back to the UI when it's in use, the same "sandbox mode" the payment step shows

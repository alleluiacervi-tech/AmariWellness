...
**P1.4 (part b) — Client OTP auth, health acknowledgement, real public booking flow. Not started, gated the same way as P1.3 part 1 above** — a real `/book` can't stay static (a booking has to hit the database to check availability and reserve), so swapping it in from the always-static preview carries the same production risk the note above documents. Build and verify locally first; hold the merge/deploy of the public-page swap until `DATABASE_URL` is confirmed wired in Vercel.
- [ ] `src/server/client-auth/`: phone + one-time code (an SMS/WhatsApp adapter, `console` for now — same adapter pattern as `payments/`), database-backed client session, DAL
- [ ] Real `/book`: session → live day/time (`getDaySlots`) → phone + code → health acknowledgement (first booking only) → a sandbox "pay" step that simulates the provider confirming → confirmation with the real booking reference and QR
- [ ] Real `/account`: phone + code sign-in, real bookings and QR codes in place of the sample data

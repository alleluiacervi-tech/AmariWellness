...
> 3. **The four staff accounts from `pnpm db:seed` share a password that's written in this public repository** (`change-me-now`, unless `SEED_STAFF_PASSWORD` was set when seeding), and whoever signs in first sets up that account's two-step verification. If production was seeded, sign in to all four at `/staff/login` now with your own authenticator app, which claims them. There's no way yet to change a staff password from the app, and re-running the seed doesn't change existing accounts.

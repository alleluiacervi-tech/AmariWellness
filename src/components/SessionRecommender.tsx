...
const REASONS: Record<string, string> = {
  quick: 'Short and targeted at the top of your spine — it fits in a lunch break and still makes a difference.',
  ...
  'partner-30': 'Two private rooms side by side, running at the same time. You each set your own programme.',
}
...
export function SessionRecommender() {
  ...
  const rec = recommend()
  const answered = time || focus

  return (
    <div className="recommender-card">
      <div className="recommender-head">
        <p className="label">Not sure which</p>
        <h2 className="recommender-title">Two questions, and we will tell you.</h2>
      </div>

      <div className="recommender-steps">
        <div className="recommender-step">
          <span className="recommender-step-label">How much time do you have?</span>
          <div className="recommender-options">
            {TIME_OPTIONS.map(opt => (
              <button
                ...
                className={`recommender-btn${time === opt.id ? ' recommender-btn--selected' : ''}`}
        ...
        <div className="recommender-step">
          <span className="recommender-step-label">What is bothering you?</span>
          <div className="recommender-options">
            {FOCUS_OPTIONS.map(opt => (
              <button
                ...
                className={`recommender-btn${focus === opt.id ? ' recommender-btn--selected' : ''}`}
      ...
      {answered && (
        <div className="recommender-result" role="status" aria-live="polite">
          <div className="recommender-result-info">
            <span className="label" style={{ color: 'var(--gold-text)' }}>Book this one</span>
          ...
          <Link href={`/book?session=${rec.id}`} className="btn btn--solid recommender-cta">
            Book it →
          </Link>
        </div>
      )}

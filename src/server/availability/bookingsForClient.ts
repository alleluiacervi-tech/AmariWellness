...
export type ClientBookingDetail = {
  ...
  /** Sums of this booking's ledger entries — the receipt shows money moved, never an editable number. */
  money: { paidRwf: number; discountRwf: number; refundedRwf: number }
}
...
export async function getClientBooking(clientId: string, bookingId: string): Promise<ClientBookingDetail | null> {
  ...
  return {
    ...
    payment: payment
      ...
      : null,
    money: { paidRwf: sum("payment_received"), discountRwf: -sum("discount_applied"), refundedRwf: -sum("refund") },
  }
}
